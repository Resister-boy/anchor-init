...
export function ClusterUiSelect() {
  ...
  return (
    <div className="dropdown dropdown-end">
      <div tabIndex={0} role="button" className="cluster-button">
        {cluster?.name || "Devnet"}
      </div>
      <ul
        tabIndex={0}
        className="dropdown-content menu p-2 mt-2 shadow bg-base-100 rounded-box w-52 z-10"
      >
        {clusters.map((item) => (
          <li key={item.name}>
            <button
              className={`${
                item.name === cluster?.name ? "bg-primary text-white" : ""
              }`}
